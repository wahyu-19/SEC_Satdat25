...
def proses_peramalan(file):
    ...
    # ==================================================
    # PREPROCESSING DATA (AMAN)
    # ==================================================
    df_proc = df.copy()
    df_proc['RR'] = df_proc['RR'].replace([8888, 9999, "-"], np.nan)
    df_proc['RR'] = pd.to_numeric(df_proc['RR'], errors='coerce')
    ...
    dataset = df_proc['RR_norm'].values.reshape(-1, 1)
    look_back = 30
